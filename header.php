<html>

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Chela+One&family=Roboto&display=swap" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css" />
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="responsive.css">

    <script>
        function myFunction(x) {
            x.classList.toggle("change");
        }
    </script>

    <link rel="icon" type="image/x-icon" href="IMG/favicon icon.jpg">
</head>

<body>
    <header>
        <div class="logoimg">
            <img src="IMG/logo.png" alt="logo" class="logo">
        </div>
        <nav>
            <div class="underlineEffects">
                <ul class="navbar">
                    <li><a href="index.php">Home</a></li>
                    <li class="about-sub">
                        <a href="#">
                            About <span class="down-icon"><i class='bx bxs-chevron-down'></i></span>
                        </a>
                        <div class="sub">
                            <ul class="sub-menu">
                                <li class="sub-menu-item">
                                    <a href="temple.php">Temple</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="whatiskcon.php">What is ISKCON?</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="whoissrila.php">Who is Srila Prabhupada?</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="hhmaha.php">About HH Mahavishnu Goswami
                                        Maharaja</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="authority.php">Authority</a>
                                </li>
                            </ul>
                        </div>
                    </li>
                    <li class="activity-sub">
                        <a href="#">
                            Our Activities <span class="down-icon"><i class='bx bxs-chevron-down'></i></span>
                        </a>
                        <div class="sub-activity">
                            <ul class="sub-menu">
                                <li class="sub-menu-item">
                                    <a href="iyf.php">ISKCON Youth Forun(IYF)</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="sunday.php">Sunday Program</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="prahlad.php">Sri Prahlad Children's Class</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="radharani.php">Radha Rani Sabha</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="course-bhagvad.php">Courses On Bhagvad Gita</a>
                                </li>
                            </ul>
                        </div>
                    </li>
                    <li><a href="lectures.php">Lectuers</a></li>
                    <li class="booking-sub">
                        <a href="#">
                            Booking<span class="down-icon"><i class='bx bxs-chevron-down'></i></span>
                        </a>
                        <div class="sub-booking">
                            <ul class="sub-menu">
                                <li class="sub-menu-item">
                                    <a href="guest-house.php">Guest House Booking</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="prasadam.php">Prasadam Booking</a>
                                </li>
                            </ul>
                        </div>
                    </li>
                    <li class="donate-sub">
                        <a href="#">
                            Donate<span class="down-icon"><i class='bx bxs-chevron-down'></i></span>
                        </a>
                        <div class="sub-donate">
                            <ul class="sub-menu">
                                <li class="sub-menu-item">
                                    <a href="gau-seva.php">Gau Seva</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="nitya-seva.php">Nitya Seva</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="sadhu.php">Sadhu Bojan</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="daily-anand.php">Daily Annadan</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="food.php">Food to the NEEDY</a>
                                </li>
                                <li class="sub-menu-item">
                                    <a href="donatepolicy.php">Donation Policy</a>
                                </li>
                            </ul>
                        </div>
                    </li>
                    <li><a href="#">Gallery</a></li>
                    <li><a href="#">
                            Contact Us
                        </a></li>
                </ul>

            </div>
        </nav>
    </header>